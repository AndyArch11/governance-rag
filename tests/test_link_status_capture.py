...
def test_link_status_captured_in_metadata():
    ...
    for link_status, expected_link_score in test_cases:
        ...
        assert node.link_status == link_status, f"Expected link_status={link_status}, got {node.link_status}"
...
def test_link_status_affects_quality_score():
    ...
    for i, link_status in enumerate(["available", "stale_404", "stale_timeout", "stale_moved", "unresolved"]):
    ...
    assert scores["stale_moved"] > scores["stale_timeout"], "stale_moved should score higher than stale_timeout"
    assert scores["available"] > scores["unresolved"], "available should score higher than unresolved"
