...
class DummyCollection:
    ...
    def query(self, query_embeddings=None, query_texts=None, n_results=None, where=None, include=None):  # noqa: D401
        """Mock vector search."""
...
class TestRetrieveSuccess:
    ...
    def test_retrieve_handles_empty_results(self, retrieve_module):
        retrieve, logger, audit_events = retrieve_module
        empty_collection = DummyCollection(results={
            "ids": [[]],
            "documents": [[]],
            "metadatas": [[]],
            "distances": [[]]
...
class TestRetrieveErrors:
    ...
    def test_retrieve_handles_search_failures_gracefully(self, monkeypatch, retrieve_module):
        ...
        assert any("warning" in msg.lower() or "fail" in msg.lower() 
    ...
    def test_keyword_search_receives_filters(self, retrieve_module, monkeypatch):
        ...
        def mock_get(where=None, limit=None, include=None, ids=None):
            ...
            if include and 'embeddings' in include:
                return {"ids": ["dummy"], "documents": ["dummy"], "metadatas": [{}], "embeddings": [[0.1] * 1024]}
        ...
        keyword_calls = [c for c in get_calls if c.get("where") and c["where"].get("language") == "java"]
        assert len(keyword_calls) > 0, f"Expected keyword search call with language filter, all calls: {get_calls}"
        ...
        assert get_where_value(where_clause, "language") == "java", f"Expected language='java', got {where_clause}"
        assert get_where_value(where_clause, "chunk_type") == "child", f"Expected chunk_type='child', got {where_clause}"
