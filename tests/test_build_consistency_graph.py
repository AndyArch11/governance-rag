...
class TestClusters:
    ...
    def test_generate_cluster_metadata_invokes_llm_label_cluster(self, monkeypatch):
        ...
        called = {}
        def fake_llm_label_cluster(docs, severities, topics, cluster_type, is_code_cluster=False, code_languages=None, code_services=None):
            called['docs'] = docs
    ...
    def test_generate_cluster_metadata_code_cluster(self, monkeypatch):
        G = nx.Graph()
        G.add_node("c1", doc_id="svc1", version=1, summary="auth service", source_category="code", language="java", service="auth")
        G.add_node("c2", doc_id="svc2", version=1, summary="payment service", source_category="code", language="java", service="payment")
        clusters = [set(["c1", "c2"])]
...
class TestParallelGraphBuild:
    def test_build_consistency_graph_parallel_uses_stubbed_edges(self, monkeypatch):
        ...
        def stub_process(record, doc_collection, max_neighbours, sim_threshold, enable_heuristic=True, embedding_cache=None, llm_batcher=None, enable_dynamic_expansion=True, expansion_quality_threshold=0.7, max_expansion_multiplier=1.5):
            if record["doc_id"] == "a":
                return [
                    {
                        "source": "a_v1",
                ...
                ], {"llm_calls": 1, "filtered": 0, "cache_hits": 0, "cache_misses": 0, "expansion_applied": False, "expanded_neighbours_found": 0}
            return [], {"llm_calls": 0, "filtered": 0, "cache_hits": 0, "cache_misses": 0, "expansion_applied": False, "expanded_neighbours_found": 0}
...
class TestDocTypeFilter:
    def test_filter_doc_type_passes_correct_where(self, monkeypatch):
        ...
        def fake_load_versioned_docs(collection, batch_size=500, where=None, include_documents=True):
            called['where'] = where
        ...
        def fake_to_networkx(graph):
            G = nx.Graph()
            G.add_node("x_v1", doc_id="x", version=1, conflict_score=0.0, risk_clusters=[0], topic_clusters=[0])
            return G
        monkeypatch.setattr(bcg, "to_networkx", fake_to_networkx)
        # Patch generate_cluster_metadata to avoid downstream errors
        monkeypatch.setattr(bcg, "generate_cluster_metadata", lambda G, clusters, cluster_type: {0: {"label": "L", "description": "D", "summary": "S", "size": 1}})
        ...
        monkeypatch.setattr(bcg, "logger", type("Logger", (), {"info": lambda *a, **k: None, "error": lambda *a, **k: None, "warning": lambda *a, **k: None})())
...
class TestOptimisations:
    ...
    def test_sample_and_interpolate_graph_samples_correctly(self):
        ...
        versioned_docs = [
            {"doc_id": f"doc{i}", "version": 1, "embedding": [float(i)],
             "summary": f"Summary {i}", "doc_type": "test", "source_category": "test",
             "health": {}, "text": f"Text {i}"}
            for i in range(10)
        ]
        ...
        def mock_build(*args, **kwargs):
            return {
                "nodes": {f"doc{i}_v1": {"doc_id": f"doc{i}", "version": 1}
                          for i in range(10)},
                "edges": [],
                "clusters": {"risk": {}, "topic": {}},
            }
        ...
        with mock.patch("scripts.consistency_graph.build_consistency_graph.build_consistency_graph_parallel", mock_build):
    ...
    def test_build_consistency_graph_parallel_with_batching(self, monkeypatch):
        ...
        def stub_process(record, doc_collection, max_neighbours, sim_threshold,
                        enable_heuristic=True, embedding_cache=None, llm_batcher=None, enable_dynamic_expansion=True, expansion_quality_threshold=0.7, max_expansion_multiplier=1.5):
