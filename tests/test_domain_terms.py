...
class TestDomainTerm:
    ...
    def test_domain_term_optional_fields(self):
        ...
        term = DomainTerm(term="security", category="general")
...
class TestDomainTermManager:
    ...
    def test_expand_with_domain_terms(self):
        ...
        terms, weights = manager.expand_with_domain_terms("encryption", DomainType.CYBERSECURITY)
    ...
    def test_load_from_json(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            vocab_data = {
                ...
                "terms": [
                    {"term": "test_term", "acronym": "TT", "category": "test", "weight": 2.0}
                ],
            }
    ...
    def test_merge_vocabularies(self):
        ...
        if vocab1:
            ...
            assert (
                manager.merge_vocabularies(DomainType.CYBERSECURITY, DomainType.CYBERSECURITY)
                == True
            )
...
class TestCandidateTerms:
    ...
    def test_approve_candidate_term_adds_to_vocab(self, tmp_path: Path):
        ...
        approved = manager.approve_candidate_term(
            "data residency", category="compliance", weight=1.8, curated_by="tester"
        )
    ...
    def test_domain_vocabulary_completeness(self):
        ...
        domains = [
            ...
            (
                ...
                "rag_data/domain_terms/cloud_infrastructure.json",
                10,
            ),
        ...
        for domain_type, path, min_terms in domains:
            ...
            if path_obj.exists():
                ...
                assert vocab is not None
                assert (
                    len(vocab.terms) >= min_terms
                ), f"{domain_type.value} has only {len(vocab.terms)} terms, expected at least {min_terms}"
...
class TestDomainTermWeighting:
    ...
    def test_critical_terms_higher_weight(self):
        ...
        if cyber and len(cyber.terms) > 0:
            ...
            encryption_terms = [t for t in cyber.terms.values() if "encryption" in t.term.lower()]
            if encryption_terms:
    ...
    def test_regulatory_term_weights(self):
        ...
        if legal and len(legal.terms) > 0:
            gdpr_terms = [t for t in legal.terms.values() if "GDPR" in (t.acronym or "")]
