...
def test_no_aggressive_s_removal():
    ...
    assert any(
        "torres" in t.lower() and "torre " not in t.lower() for t in term_list
    ), f"Torres should not be changed to Torre. Got terms: {term_list}"

    assert any(
        "indigenous" in t.lower() for t in term_list
...
def test_plural_forms_preserved():
    ...
    for term in term_list:
        assert (
            not term.rstrip() != term
        ), f"Term has trailing spaces (sign of truncation): {repr(term)}"
