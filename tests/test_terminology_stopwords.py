...
class TestDomainStopWords:
    ...
    def test_metadata_artifacts_excluded(self):
        ...
        assert metadata_artifacts.issubset(
            DOMAIN_STOP_WORDS
    ...
    def test_pdf_footer_artifacts_excluded(self):
        ...
        assert footer_artifacts.issubset(
            DOMAIN_STOP_WORDS
...
class TestTextPreprocessorStopWords:
    ...
    def test_tokenise_filters_stop_words(self):
        ...
        text = (
            "Click here to save print post share documents none unknown null research paper study"
        )
    ...
    def test_tokenise_removes_publisher_names(self):
        ...
        filtered_publishers = [
            t
            for t in tokens
            if t in {"springer", "elsevier", "nature", "science", "google", "scholar"}
        ]
...
class TestDomainTerminologyExtractor:
    ...
    def test_extract_from_academic_text(self):
        ...
        stop_word_terms = [
            ...
            if t in DOMAIN_STOP_WORDS or any(sw in t.split() for sw in DOMAIN_STOP_WORDS)
        ]

        # Allow only a few stop words due to n-gram extraction edge cases
        assert len(stop_word_terms) < 3, f"Too many stop words in terms: {stop_word_terms}"
...
class TestStopWordIntegration:
    ...
    def test_terminology_extractor_uses_additional_stopwords(self):
        ...
        assert DOMAIN_STOP_WORDS.issubset(
            extractor.preprocessor.stopword_set
