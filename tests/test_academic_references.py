...
class TestIsSubterm:
    ...
    def test_multi_word_subterm(self):
        ...
        # Two-word subterm
        assert module._is_subterm(
            "torres strait", "aboriginal torres strait"
        ), "Two-word term should be identified as subterm"

        assert module._is_subterm(
            "strait islander", "torres strait islander"
        ), "Ending subterm should be identified"

        assert module._is_subterm(
            "aboriginal torres", "aboriginal torres strait"
    ...
    def test_non_contiguous_words_not_subterm(self):
        ...
        assert not module._is_subterm(
            "aboriginal strait", "aboriginal torres strait"
        ), "Non-contiguous words should not be identified as subterm"

        assert not module._is_subterm(
            "torres islander", "torres strait islander"
        ), "Non-contiguous words should not be identified as subterm"
    ...
    def test_longer_term_not_subterm(self):
        ...
        assert not module._is_subterm(
            "aboriginal torres strait", "torres strait"
        ), "Longer term should not be subterm of shorter one"

        assert not module._is_subterm(
            "torres strait islander", "torres strait"
    ...
    def test_case_sensitive_matching(self):
        ...
        assert module._is_subterm("torres", "torres strait"), "Case-sensitive matching should work"

    def test_empty_terms(self):
        """Test handling of empty terms."""
        module = AcademicReferences()

        assert not module._is_subterm("", "torres strait"), "Empty term should not be subterm"

        assert not module._is_subterm("torres", ""), "No parent term to check against"
...
class TestAcademicReferencesModule:
    ...
    def test_get_top_terms_with_real_database(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                result[0]["term"] == "aboriginal torres strait islander"
            ), "Top term should be the full phrase"
...
class TestSubtermFilteringScenarios:
    ...
    def test_non_overlapping_terms_preserved(self):
        ...
        assert not module._is_subterm(term1, term2), "Non-overlapping terms should not be filtered"

        assert not module._is_subterm(term2, term1), "Non-overlapping terms should not be filtered"
