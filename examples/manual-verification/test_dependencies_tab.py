...
def test_dependency_extraction():
    ...
    for i, (node_id_a, data_a) in enumerate(code_nodes.items()):
        deps_a = set(data_a.get("dependencies", []) if isinstance(data_a.get("dependencies"), list) else [])
        for node_id_b, data_b in list(code_nodes.items())[i+1:]:
            deps_b = set(data_b.get("dependencies", []) if isinstance(data_b.get("dependencies"), list) else [])
            shared = deps_a & deps_b
            if shared:
                if not dep_graph.has_edge(node_id_a, node_id_b):
                    weight = len(shared) / max(len(deps_a), len(deps_b)) if max(len(deps_a), len(deps_b)) > 0 else 0
                    dep_graph.add_edge(node_id_a, node_id_b, type="shared_deps", weight=weight, shared=sorted(list(shared)))
                    shared_deps_count += 1
    ...
    if cycles:
        ...
        for i, cycle in enumerate(cycles[:5], 1):
            cycle_services = [code_nodes[nid].get("service_name", nid) for nid in cycle if nid in code_nodes]
