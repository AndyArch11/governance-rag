...
from scripts.utils.db_factory import get_cache_client, get_vector_client

# Set up logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler("/tmp/bm25_rebuild.log")],
)
...
def main():
    ...
    try:
        ...
        while offset < total_docs:
            try:
                ...
                for i, (doc_id, text, metadata) in enumerate(
                    zip(batch["ids"], batch["documents"], batch["metadatas"])
                ):
                    try:
                        if not text:
        ...
        if corpus_size > 0:
            ...
            logger.info(
                f"✓ BM25 corpus stats updated: {corpus_size} documents, avg length {avg_doc_len:.1f} tokens"
            )
        else:
