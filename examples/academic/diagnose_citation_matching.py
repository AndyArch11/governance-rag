...
for row in cursor:
    print(
        f"  [{row['node_type']}] {row['node_id']}: {row['title'][:60] if row['title'] else 'N/A'}..."
    )
...
cursor.execute(
    "SELECT node_id, node_type, title FROM nodes WHERE node_id LIKE '%academic%' OR node_id LIKE '%2025%' OR title LIKE '%2025%' LIMIT 10"
)
...
if matches:
    ...
    for row in matches:
        print(
            f"  [{row['node_type']}] {row['node_id']}: {row['title'][:80] if row['title'] else 'N/A'}"
        )
else:
